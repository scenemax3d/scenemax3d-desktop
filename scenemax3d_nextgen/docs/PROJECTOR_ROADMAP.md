...
Target folder: `C:\dev\scenemax_desktop\scenemax3d_nextgen`
...
- `NextGen`: runs the new Bevy/Rust projector in `C:\dev\scenemax_desktop\scenemax3d_nextgen`
...
- `scenemax_projector_nextgen`: executable entry point in `Projector/app`.
...
Current component layout (shared with the independent IDE):
...
The local ANTLR tool is stored at `tools/antlr/antlr4-4.8-2-SNAPSHOT-complete.jar`; grammar generation is handled by `Language/parser/build.rs`.
