...
struct SceneMaxEffekseerRenderer {
  ...
  std::unordered_map<uint64_t, SceneMaxEffekseerPreviewOptions> previewOptions;
  std::unordered_map<uint64_t, SceneMaxEffekseerPreviewOptions> appliedOptions;
...
uint64_t playInstance(SceneMaxEffekseerRenderer* ctx,
  ...
  auto options = ctx->previewOptions.find(instance->id);
  if (options != ctx->previewOptions.end()) {
    ctx->manager->SetRandomSeed(handle, options->second.seed);
    ctx->appliedOptions.erase(instance->id);
  }
...
scenemax_effekseer_render_vulkan(SceneMaxEffekseerRenderer* renderer,
  ...
  for (const auto& entry : renderer->previewOptions) {
    auto handle = renderer->handles.find(entry.first);
    if (handle == renderer->handles.end() || !renderer->manager->Exists(handle->second)) continue;
    const auto& options = entry.second;
    renderer->manager->SetTargetLocation(handle->second, options.target[0], options.target[1], options.target[2]);
    renderer->manager->SetAllColor(handle->second, {options.color[0], options.color[1], options.color[2], options.color[3]});
    auto previous = renderer->appliedOptions.find(entry.first);
    if (options.seek_generation != 0 && (previous == renderer->appliedOptions.end() || previous->second.seek_generation != options.seek_generation)) {
      renderer->manager->UpdateHandleToMoveToFrame(handle->second, std::clamp(options.seek_frame, 0.0f, 36000.0f));
      renderer->simulationTime = options.seek_frame / 60.0f;
    }
    for (int i = 0; i < 4; ++i) {
      if (options.triggers[i] != 0 && previous != renderer->appliedOptions.end() && previous->second.triggers[i] != options.triggers[i]) renderer->manager->SendTrigger(handle->second, i);
    }
    renderer->appliedOptions[entry.first] = options;
  }
  ...
  renderer->simulationTime += std::max(delta_seconds, 0.0f);
  renderer->renderer->SetTime(renderer->simulationTime);
...
extern "C" SCENEMAX_EFFEKSEER_API void scenemax_effekseer_preview_options(
    SceneMaxEffekseerRenderer* renderer, uint64_t id, const SceneMaxEffekseerPreviewOptions* options) {
  if (renderer != nullptr && options != nullptr) renderer->previewOptions[id] = *options;
}
