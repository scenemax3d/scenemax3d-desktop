...
- how project and global settings work
...
The itch.io target is configured per project.
...
- `itch.io API Key`
  - Optional project-scoped credential for Butler uploads
...
`File > Settings... > Butler`

Butler executable setup and local login are configured once for the SceneMax installation.
...
- `Butler Login...`
  - Starts the local `butler login` flow for the current machine
...
6. SceneMax finds `butler.exe`, fills the field automatically, and saves that path globally when you save Settings
...
- click `Butler Login...` in Settings > Butler
...
If you prefer not to use `butler login`, you can paste a project API key into the `itch.io API Key` field in Project Settings.
...
4. Open `Settings > Butler`
5. Browse to the downloaded `butler` zip
6. Click `Butler Login...`
7. Save the settings
8. Package your Windows, Linux, or macOS build
9. Enable automatic itch.io upload
...
- Project page and channel names are stored with project settings
- Butler executable path is stored globally in SceneMax settings
...
2. install or detect `butler` in Settings > Butler
