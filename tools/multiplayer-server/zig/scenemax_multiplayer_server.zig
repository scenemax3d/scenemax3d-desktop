...
pub fn main(init: std.process.Init) !void {
    ...
    while (true) {
        ...
        const station_id = std.mem.readInt(u16, buffer[6..][0..2], .little);
        ...
        switch (packet_type) {
            .login_request => {
                ...
                const assigned = next_station_id;
                next_station_id +%= 1;
                if (next_station_id == 0) next_station_id = 1;
                const slot = assigned % max_stations;
                stations[slot] = .{
                    .active = true,
                    .id = assigned,
            ...
            .heartbeat => {
                if (findStation(&stations, station_id)) |station| {
                    station.last_seen_ms = now;
                    station.address = message.from;
                }
            },
            .join_scene => {
                if (findStation(&stations, station_id)) |station| {
                    station.last_seen_ms = now;
                    station.address = message.from;
                    zero(&station.scene_id);
                    copyFixed(&station.scene_id, payload, 0);
                    clearStationEntitiesInOtherScenes(&entities, station.id, station.session_id, station.scene_id);
                    try sendSnapshot(sock, io, message.from, &entities, station.session_id, station.scene_id);
                }
            },
            .create_entity_request => {
                const station = findStation(&stations, station_id) orelse continue;
                const entity_id = next_entity_id;
                next_entity_id +%= 1;
                if (next_entity_id == 0) next_entity_id = 1;
                const slot = entity_id % max_entities;
                entities[slot] = decodeEntityCreate(payload, entity_id, station.*);
                try sendEntityAccepted(sock, io, message.from, station_id, entity_id);
                try dispatch(sock, io, &stations, station_id, packet_type, station_id, payload);
            },
            .command_dispatch, .transform_correction => {
                if (findStation(&stations, station_id) == null) continue;
...
fn findStation(stations: *[max_stations]Station, station_id: u16) ?*Station {
    for (stations) |*station| {
        if (station.active and station.id == station_id) return station;
    }
    return null;
}

fn expireStations(stations: *[max_stations]Station, now: i64) void {
    for (stations) |*station| {
        if (station.active and now - station.last_seen_ms > 15000) {
            station.active = false;
        }
    }
}

fn decodeEntityCreate(payload: []const u8, entity_id: u32, station: Station) Entity {
    var entity = Entity{
        .active = true,
        .network_id = entity_id,
        .owner_station = station.id,
...
fn clearStationEntitiesInOtherScenes(entities: *[max_entities]Entity, station_id: u16, session_id: u32, scene_id: [128]u8) void {
    for (entities) |*entity| {
        if (entity.active and entity.owner_station == station_id and entity.session_id == session_id and !sameScene(entity.scene_id, scene_id)) {
            entity.active = false;
        }
    }
}
...
fn sendLoginAccepted(sock: net.Socket, io: std.Io, address: net.IpAddress, station_id: u16, session_id: u32, session_name: [64]u8) !void {
    var packet: [78]u8 = undefined;
    writeHeader(packet[0..], .login_accepted, station_id);
    std.mem.writeInt(u16, packet[8..][0..2], station_id, .little);
    std.mem.writeInt(u32, packet[10..][0..4], session_id, .little);
    @memcpy(packet[14..78], &session_name);
    try sock.send(io, &address, &packet);
}

fn sendEntityAccepted(sock: net.Socket, io: std.Io, address: net.IpAddress, station_id: u16, entity_id: u32) !void {
    var packet: [12]u8 = undefined;
    writeHeader(packet[0..], .create_entity_accepted, station_id);
...
fn sendSnapshot(sock: net.Socket, io: std.Io, address: net.IpAddress, entities: *[max_entities]Entity, session_id: u32, scene_id: [128]u8) !void {
    ...
    for (entities) |entity| {
        ...
        if (!entity.active or cursor + 176 > packet.len) continue;
        std.mem.writeInt(u32, packet[cursor..][0..4], entity.network_id, .little);
        cursor += 4;
        std.mem.writeInt(u16, packet[cursor..][0..2], entity.owner_station, .little);
        cursor += 2;
...
fn dispatch(sock: net.Socket, io: std.Io, stations: *[max_stations]Station, sender_id: u16, packet_type: PacketType, station_id: u16, payload: []const u8) !void {
    var packet: [max_packet_size]u8 = undefined;
    writeHeader(packet[0..], packet_type, station_id);
    const payload_len = @min(payload.len, packet.len - 8);
    @memcpy(packet[8 .. 8 + payload_len], payload[0..payload_len]);
    const sender = findStation(stations, sender_id) orelse return;
    for (stations) |station| {
        if (!station.active or station.id == sender_id) continue;
        if (station.session_id != sender.session_id or !sameScene(station.scene_id, sender.scene_id)) continue;
        try sock.send(io, &station.address, packet[0 .. 8 + payload_len]);
    }
}
...
fn writeHeader(packet: []u8, packet_type: PacketType, station_id: u16) void {
    std.mem.writeInt(u32, packet[0..][0..4], protocol_magic, .little);
    packet[4] = protocol_version;
    packet[5] = @intFromEnum(packet_type);
    std.mem.writeInt(u16, packet[6..][0..2], station_id, .little);
}
