...
public class DesignerArrayEntitiesTool extends AbstractSceneMaxTool {

    @Override
    public String getName() {
        return "designer.array_entities";
    }

    @Override
    public String getDescription() {
        return "Creates an array of duplicated scene entities using a repeated step offset.";
    }
    ...
    public JSONObject getInputSchema() {
        return new JSONObject()
                ...
                .put("properties", new JSONObject()
                        ...
                        .put("step", new JSONObject().put("type", "array"))
                ...
                .put("required", new JSONArray().put("count").put("step"));
    }
    ...
    public SceneMaxToolResult execute(SceneMaxToolContext context, JSONObject arguments) throws Exception {
        ...
        int count = Math.max(1, optionalInt(arguments, "count", 1));
        JSONArray step = arguments.optJSONArray("step");
        if (step == null || step.length() < 3) {
            throw new IllegalArgumentException("step must be an array of [x, y, z].");
        }
        double dx = step.optDouble(0, 0d);
        double dy = step.optDouble(1, 0d);
        double dz = step.optDouble(2, 0d);
        String suffixBase = optionalString(arguments, "name_suffix", "_array");
        ...
        return SceneMaxToolResult.success("Created " + created.length() + " arrayed entity copy/copies.", data);
    }
}
